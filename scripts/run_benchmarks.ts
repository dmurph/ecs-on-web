...
async function runBenchmarkForBehavior(
  ...
  let sims = [
    ...
    {
      instance: new CustomECSSimulator(SortMethod.Insertion),
      name: 'Custom ECS S&P (Insertion)',
    },
    {
      instance: new CustomECSSimulator(SortMethod.Quick),
      name: 'Custom ECS S&P (Quick)',
    },
    {
      instance: new CustomECSSimulator(SortMethod.Merge),
      name: 'Custom ECS S&P (Merge)',
    },
    {
      instance: new CustomECSSimulator(SortMethod.Native),
      name: 'Custom ECS S&P (Native)',
    },
    { instance: new ECSTreeSimulator(), name: 'ECS Tree' },

    {
      instance: new BitECSSimulator('insertion'),
    ...
    {
      instance: new WasmECSSimulator(SortMethod.Insertion),
      name: 'WASM ECS S&P (Insertion)',
    },
    {
      instance: new WasmECSSimulator(SortMethod.Quick),
      name: 'WASM ECS S&P (Quick)',
    },
    {
      instance: new WasmECSSimulator(SortMethod.Merge),
  ...
  console.log(
    `| System | Avg Frame Time | 99th Percentile | Speedup vs OOP S&P (Insertion) |`,
  );
  console.log(`| :--- | :--- | :--- | :--- |`);
  results.forEach((r) => {
...
async function start() {
  ...
  const customEcsModule =
    await import('../src/benchmarks/benchmark_custom_ecs');
